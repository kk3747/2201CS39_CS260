...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) 
{
    ...
    $sql = "SELECT * FROM `registered_data` WHERE `APP_NO` = ?";
    ...
    $sql = "SELECT * FROM `curr_job` WHERE `APP_NO` = ?;";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $app_no );
    $stmt->execute();
    ...
    $stmt->close();
    $sql = "SELECT * FROM `emp_hist_data` WHERE `APP_NO` = ?;";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $app_no );
    $stmt->execute();
    $result_prev = $stmt->get_result();   
    ...
    mysqli_free_result($result_prev);
    $stmt->close();    
    $sql = "SELECT * FROM `teaching_exp` WHERE `APP_NO` = ?;";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $app_no );
    $stmt->execute();
    $result_prev = $stmt->get_result();
    ...
    mysqli_free_result($result_prev);
    $stmt->close();
    $sql = "SELECT * FROM `research_exp` WHERE `APP_NO` = ?;";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $app_no );
    $stmt->execute();
    $result_prev = $stmt->get_result();
    ...
    if( $result_prev->num_rows > 0 ) 
    {
      ...
      mysqli_free_result($result_prev);
      $stmt->close();
      $sql = "SELECT * FROM `exp_of_industry` WHERE `APP_NO` = ?;";
      $stmt = $conn->prepare($sql);
      $stmt->bind_param("i", $app_no );
      $stmt->execute();
      $result_prev = $stmt->get_result();
      ...
      $sql = "SELECT * FROM `main_skill` WHERE `APP_NO` = ?;";
      $stmt = $conn->prepare($sql);
      $stmt->bind_param("i", $app_no);     
      if($stmt->execute())
      {       
      ...
      $sql = "INSERT INTO curr_job (APP_NO, position, status , 	DateOfLeaving, OrganizationInstitution	, DateOfJoining, DurationYears ) 
      VALUES (?, ?, ?, ?, ?, ?, ?)
      ...
      $sql = "INSERT INTO `curr_job` ( Position, Organization, Date_of_Joining, Date_of_Leaving, duration , APP_NO , id) VALUES (?, ?, ?, ?, ?, ? , ?) 
      ON DUPLICATE KEY UPDATE 
      APP_NO = VALUES(APP_NO), 
      Position = VALUES(Position), 
      ...
      for ($i = 0; !is_null($positions) &&  $i < count($positions); $i++) 
      {
        ...
        $sql = "INSERT INTO `teaching_exp` ( id , app_no , position, employer , CourseTaught	, 	UG_PG , 	NoOfStudents , DateOfJoining, DateOfLeaving	, Duration) VALUES (?, ?, ?, ?, ?, ? , ? , ?, ? , ?) 
        ON DUPLICATE KEY UPDATE 
        id = VALUES(id),
        app_no = VALUES(app_no), 
        position = VALUES(position), 
        ...
        $sql = "INSERT INTO `research_exp` (id, app_no, position, Institute, Supervisor, DateOfJoining, DateOfLeaving, duration) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE 
        id=VALUES(id),
        app_no=VALUES(app_no), 
        position=VALUES(position), 
        ...
        $sql = "INSERT INTO `exp_of_industry` (id, APP_NO, organization, WorkProfile, DateOfJoining, DateOfLeaving, Duration) VALUES (?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE 
        id=VALUES(id),
        app_no=VALUES(app_no), 
        organization=VALUES(organization), 
        ...
        $sql = "INSERT INTO `main_skill` (APP_NO , area_of_specialization , current_area_of_research) VALUES ( ?, ?, ? ) 
        ON DUPLICATE KEY UPDATE
        APP_NO = VALUES(APP_NO),
