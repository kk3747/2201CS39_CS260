...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) 
{
    ...
    $sql = "SELECT * FROM `training_experience` WHERE `APP_NO` = ?;";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $app_no );
    $stmt->execute();
    $result_prev = $stmt->get_result(); 
    ...
    $prev_a_body = [];
    $prev_a_year = [];    
    $sql = "SELECT * FROM `award_details` WHERE `APP_NO` = ?;";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $app_no );
    $stmt->execute();
    $result_prev = $stmt->get_result();
    ...
    $stmt->close();
    mysqli_free_result($result_prev);
    $sql = "SELECT * FROM `proj_info` WHERE `APP_NO` = ?;";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $app_no );
    $stmt->execute();
    $result_prev = $stmt->get_result();  
    ...
    $stmt->close();
    mysqli_free_result($result_prev);   
    $sql = "SELECT * FROM `consult` WHERE `APP_NO` = ?;";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $app_no );
    $stmt->execute();
    $result_prev = $stmt->get_result();  
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(isset($_POST['awardmemb']) ) {
                ...
                // Prepare and bind SQL statement for books
                $sql = "INSERT INTO `training_experience` (APP_NO ,  SNo , TypeOfTrainingReceived , Organisation , Year , Duration  ) VALUES (?, ?, ? , ? , ? , ? ) 
                ON DUPLICATE KEY UPDATE 
                APP_NO = VALUES(APP_NO), 
                        SNo = VALUES(SNo), 
        ...
        if( isset($_POST['awardmemb'])  ) {
                ...
                $sql = "INSERT INTO `award_details` (APP_NO ,  SNo , AwardName , AwardingBody , YearOfAward ) VALUES (?, ?, ? , ? , ? ) 
                ON DUPLICATE KEY UPDATE 
                APP_NO = VALUES(APP_NO), 
                        SNo = VALUES(SNo), 
        ...
        if( isset($_POST['stitle'])  ) {
                ...
                // Prepare and bind SQL statement for books
                $sql = "INSERT INTO `proj_info` (APP_NO ,  SNo , SponsoringAgency , TitleOfProject , SanctionedAmount , Period , Role , Status  ) VALUES (?, ?, ? , ? , ? , ? , ? , ?  ) 
                ON DUPLICATE KEY UPDATE 
                APP_NO = VALUES(APP_NO), 
                        SNo = VALUES(SNo), 
        ...
        if(  isset($_POST['ctitle']) ) {
            $org_arr_c = $_POST['c_org'];
                ...
                // Prepare and bind SQL statement for books
                $sql = "INSERT INTO `consult` (APP_NO ,  SNo , Organization , TitleOfProject , AmountGranted , Period , Role , Status  ) VALUES (?, ?, ? , ? , ? , ? , ? , ?  ) 
                ON DUPLICATE KEY UPDATE 
                APP_NO = VALUES(APP_NO), 
                        SNo = VALUES(SNo), 
